    ...
    <div v-else >
      ...
      <div v-if="!fav && !teamSelect && !type" class="cards">
        <div v-for="pokemon in pokemons" :key="pokemon.name" :class="getPokemonClasses(pokemon)" style="width: 18rem;">
          ...
          <div :class="{ 'addTeamIcon icons': !checkForTeam(pokemon), 'addTeamIcon teamIconCheck icons': checkForTeam(pokemon) }" @click="addToTeam(pokemon)"/>
      ...
      <div v-else-if="type && !fav && !teamSelect " class="cards">
        <div v-for="pokemon in this.typeSelect" :key="pokemon.name" :class="getPokemonClasses(pokemon)" style="width: 18rem;">
          <div :class="{ 'favIcon icons': !checkForFavs(pokemon), 'favIcon favIconCheck icons': checkForFavs(pokemon) }" @click="addToFav(pokemon)"> <img class="images" src="../assets/lockIcon.png" alt="" srcset=""> </div>
          <div :class="{ 'addTeamIcon icons': !checkForTeam(pokemon), 'addTeamIcon teamIconCheck icons': checkForTeam(pokemon) }" @click="addToTeam(pokemon)"/>
          <h5 class="card-title" >Number: {{ pokemon.id }}</h5>
          <img class="card-img-top" :src="pokemon.sprites.front_default" alt="Card image cap">
    ...
    methods: {  
      ...
      displayTypes(typeSelected) {
        ...
        for (let i = 0; i < this.typeSelect.length; i++) {
          this.typeSelect.splice(i);
        }
        for (let index = 0; index < this.typeSelect.length; index++) {
          console.log(this.typeSelect[index]);
          
        }
        for (const pokemon of this.pokemons) {
          let types = pokemon.types.map(type => type.type.name)
          for (const type of types) if (type == typeSelected) this.typeSelect.push(pokemon);
            // if (types[0] == typeSelected || types[1] == typeSelected) {
            //   console.log( pokemon.types.map(type => type.type.name), pokemon.name);
